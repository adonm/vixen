...
# Repeatable startup/render measurements (local fixtures + PTY first frame).
# Prints JSON + table; does not fail on timing (no thresholds yet).
bench: build
    python3 tests/bench.py ./{{ BIN }}
