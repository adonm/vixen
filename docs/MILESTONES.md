...
| `just flutter-cdp-playwright-smoke` | release/AOT Flutter host under Cage owns the sole BrowserCore and an in-process `vixen-cdp` subscriber; focused external Playwright proof obtains layout from Flutter commits, writes a stable live `dataset`, retains live token-list, inline-style, `NamedNodeMap`, attached/detached `Attr`, structural collection, and author CSSOM objects through rendered mutations, matches same-task and CDP geometry/attributes/nodes, pins each exact scene, routes pointer input through Flutter hit testing, keeps 320×240 and 480×300 targets independent, and forces renderer reset/full-resync to a byte-identical scene; no browser/compositor chrome in direct PNGs |
