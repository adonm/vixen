...
| `just gate-architecture` | leaf-crate dependency rules plus frontend rules that forbid direct leaf composition; `vixen-cdp` depends only on API/engine and the headless/FFI composition roots select its native-comparison or Flutter render backend |
