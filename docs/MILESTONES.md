...
| `just gate-flutter-shell` | Exact mise-managed Flutter 3.46.0-0.3.pre beta framework/engine revisions, Dart formatting/analysis, 48 shell/coordinator/worker/texture/input/Semantics tests, including monotonic host focus/visibility/lifecycle state, bounded `aria-controls` relationships, and native-range adjustment, live process-adjacent native bridge smoke, and 24 focused native ABI/frame/input/accessibility tests; Linux source/test evidence, not IME, native AT, lifecycle recovery, package, release, or non-Linux proof |
...
- `just gate-native-abi` proves the handwritten C ABI/header/wire/frame ownership
  ...
  BrowserCore-to-Flutter Semantics hierarchy with one non-tree relationship and
  native range actions, but not IME, complete semantics/native
