...
//!
//! The cascade-driving document model (implementing Stylo's `TNode` /
//! `TElement` traits) lands once the `style` crate is wired (Phase 3 step 2,
//! blocked on Stylo — not on crates.io, needs a Servo git dependency). Until
//! then this `Document` is the parse tree that feeds the headless CLI and the
//! WPT snapshot surface.
